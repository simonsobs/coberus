...
tshape,twcs = enmap.band_geometry(np.asarray((-70,30))*u.degree,res=0.5*u.arcmin,variant='fejer1')
mlmax = 30000

def wmap(fname,oname):
    print(fname)
    omap = cs.alm2map(cs.map2alm(enmap.read_map(fname)[0],lmax=mlmax),enmap.empty(tshape,twcs))
...
wmap_ivar("cmb_daydeep_pa6_f090_3pass_4way_coadd_ivar.fits",'map2_mask.fits',rmax=40.0)
wmap_ivar("cmb_daywide_pa6_f150_3pass_4way_coadd_ivar.fits",'map3_mask.fits',rmax=250.0)
